...
def main():
    ...
    # Example letter N: on X plane
    goal_positions = [

        [0.500, 0.000, 0.307],  # Bottom-left (X, Y, Z)
        [0.500, 0.000, 0.590],  # Top-left (X, Y, Z)
        [0.500, 0.150, 0.307],  # Bottom-right (X, Y, Z) - diagonal
        [0.500, 0.150, 0.590],  # Top-right (X, Y, Z)
        

    ### TODO: check other plannars in Moveit library
    ...
    while rclpy.ok():
        ...
        # is less than `car tesian_fraction_threshold`. (range: [0.0, 1.0], 1.0 means 100% reached)
